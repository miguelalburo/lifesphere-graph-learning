...
## `metrics/trust_gate/` — read this before any model's number

`metrics/model{1,2,3}_*/` hold the models. `metrics/trust_gate/` holds #13's three
...
| File                      | Holds                                                                 |
| ------------------------- | --------------------------------------------------------------------- |
| `structure_ablation.json` | Model 3 retrained with every `edge_index` emptied.                      |
| `degree_probe.json`       | Cox on accrual counts alone, decomposed covariate by covariate.       |
| `label_shuffle.json`      | Every model retrained on a permuted label, under two permutation schemes. |
...
`relation_split.json` is #15: model 3 re-run with `split_primary_relation=False`
and paired fold-for-fold against the recorded split run and against model 2. It
answers whether model 3's +0.0180 margin is the secondary-diagnosis channel #11's
