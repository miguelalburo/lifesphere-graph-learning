"""#15: is arm 3's margin over arm 2 the secondary-diagnosis channel, or message passing?

#12 records arm 3 beating arm 2 by **+0.0180** within-Study Harrell C, and
~85% of arm 3's advantage over arm 1 as a structural term whose interval
...
**0.5588**, higher than the full diagnosis count (0.5569). Arms 1 and 2 cannot
see it: `features/raw.py` joins from `diagnosis_primary`, one row per Subject,
and excludes `nDiagnoses` explicitly.

So arm 3 uniquely holds a prognostic channel worth 0.5588 on its own and beats
arm 2 by +0.0180. This module tests the alternative explanation — an extra
feature rather than message passing — by re-running arm 3 with
`split_primary_relation=False` and pairing it fold-for-fold against the recorded
split run and against arm 2.

**This is not a proposal to change the arm.** #11 adopted the split on
...
Subjects with several. The arm stays as it is whatever this returns; what
...
from ..models.graph.train import FoldResult, GraphArmConfig, run_fold
...
# The one field that varies between the recorded arm 3 and this probe.
VARIES = "split_primary_relation"

# The collapsed construction is cached beside arm 3's rather than over it.
...
# rebuild for whichever ran second. A probe should not leave the arm it is
...
ASYMMETRY_FOR_14 = (
    "State this wherever arm3 - arm2 is reported (#15): arm 3 holds a prognostic channel arms 1 "
    ...
    "within-Study C = 0.5588 fitted alone (#13). Arms 1 and 2 join from diagnosis_primary, one "
...
class RelationSplitConfig:
    ...
    comparison_arm: str = TABULAR_ARM
    ...
    def check_against(self, graph_config: GraphArmConfig) -> None:
        """Raise unless this config declares the run it will actually get.

        Arm, endpoint and split go through `training.check_run_identity` — the
        ...
        by arm 3's own config, so a probe file naming a different seed would
        ...
        if mismatches:
            detail = ", ".join(
                f"{name}: {PROBE} says {declared!r}, arm 3's config supplies {supplied!r}"
                for name, (declared, supplied) in sorted(mismatches.items())
            )
            raise ValueError(
                f"{PROBE} declares a run it will not get -- {detail}. The probe takes these "
                "from experiments/configs/arm3_graph.json; change the config to match."
            )
...
def probe_config(graph_config: GraphArmConfig) -> GraphArmConfig:
    """Arm 3's own config with the relation split collapsed, and nothing else touched.
    ...
    config edit in `experiments/configs/arm3_graph.json`" — editing that file in
    place would instead change the recorded arm's declared config and break the
...
def check_probe_comparable(arm: recorded.RecordedArm, graph_config: GraphArmConfig) -> None:
    """Refuse to pair unless the recorded arm 3 differs from the probe in `VARIES` alone."""
    recorded.check_comparable(
        arm, recorded.comparable_fields(probe_config(graph_config)), varies=(VARIES,)
    )


def check_comparison_arm(
    comparison: recorded.RecordedArm, graph_config: GraphArmConfig
) -> None:
    """Refuse to pair against the comparison arm unless it is the same experiment.

    Arm 2 and arm 3 are different model classes with different config
    ...
    if not comparison.config:
        raise ValueError(
            f"{comparison.arm}'s summary.json carries no config block, so there is nothing to "
            "check this run against. Re-run the arm so its config is recorded."
        )
    mismatches = {
    ...
    if mismatches:
        detail = ", ".join(
            f"{key}: {comparison.arm} recorded {was!r}, arm 3 is {now!r}"
            for key, (was, now) in sorted(mismatches.items())
        )
        raise ValueError(
            f"{comparison.arm} was not scored on the same experiment as arm 3 -- {detail}. "
...
@dataclass(frozen=True)
class Attribution:
    """How much of arm 3's margin over the comparison arm the collapsed relation costs.
    ...
    # split - unsplit: what removing the clean presence bit costs arm 3.
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "issue": "#15",
            "question": (
                "Is arm 3's margin over arm 2 the secondary-diagnosis channel rather than "
                "message passing?"
            ),
            "metric": METRIC,
            ...
            "comparison_arm": self.config.comparison_arm,
...
def _reading(verdict: str, bound: float | None) -> str:
    ...
    if verdict == ATTRIBUTED:
        return (
            "Collapsing the relation split costs arm 3 a difference the folds resolve, so at "
            "least part of arm 3's margin over the comparison arm is the secondary-diagnosis "
    ...
    if verdict == REVERSED:
        return (
            "Collapsing the relation split *improves* arm 3, which contradicts #11's own "
            ...
            "feature in the study). Treat this as a finding about the arm rather than about the "
    ...
    return (
        ...
        "secondary-diagnosis channel is not established as the explanation for arm 3's margin. "
        ...
        "report arm 3's margin as surviving this control while stating the asymmetry, not as "
        "cleared of it."
    )
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            **self.attribution.to_dict(),
            "description": (
                "Arm 3 retrained with split_primary_relation=False and paired fold-for-fold "
                "against the recorded split run and against the comparison arm. Identical "
...
def run_relation_split(
    graph_config: GraphArmConfig,
    config: RelationSplitConfig | None = None,
    *,
    records: cache.SubjectRecords | None = None,
    raw: pd.DataFrame | None = None,
    targets: SurvivalTarget | None = None,
    graph_arm: recorded.RecordedArm | None = None,
    comparison: recorded.RecordedArm | None = None,
    cache_dir: Path | None = None,
    on_fold: Callable[[FoldResult], None] | None = None,
    use_cache: bool = True,
) -> RelationSplitResult:
    """Train the collapsed arm across all 5 folds and attribute arm 3's margin.

    The split side and the comparison side are both read from `results/metrics/`
    rather than retrained: an arm's fold is deterministic given its config and
    ...
    arm3 = graph_arm if graph_arm is not None else recorded.load_arm(GRAPH_ARM)
    comparison_arm = (
        comparison if comparison is not None else recorded.load_arm(probe.comparison_arm)
    )
    check_probe_comparable(arm3, graph_config)
    check_comparison_arm(comparison_arm, graph_config)
    ...
    for outer_fold in range(N_SPLITS):
        result = run_fold(
            ...
            use_cache=use_cache,
            # Kept on, unlike the structure ablation's. This is arm 3 with one
            ...
            # raise exactly as it would for the arm itself (#3 §6).
    ...
    return RelationSplitResult(
        attribution=assess_attribution(
            split_per_fold=arm3.metric(METRIC),
            unsplit_per_fold=[float(f.fold_metrics.within_study.cindex) for f in unsplit],
            comparison_per_fold=comparison_arm.metric(METRIC),
            config=probe,
        ),
        unsplit_config=unsplit_config,
        split_parameters=_recorded_parameter_counts(arm3),
        unsplit_parameters=tuple(_n_parameters(f.construction) for f in unsplit),
        unsplit_folds=tuple(unsplit),
    )


def _recorded_parameter_counts(arm: recorded.RecordedArm) -> tuple[int, ...]:
    """Each recorded fold's encoder size, or empty if the arm did not record one.
...
DEFAULT_ARM_CONFIG = REPO_ROOT / "experiments" / "configs" / f"{GRAPH_ARM}.json"
DEFAULT_OUT = REPO_ROOT / "results" / "metrics" / PROBE


def main(
    config_path: Path = DEFAULT_CONFIG,
    arm_config_path: Path = DEFAULT_ARM_CONFIG,
    ...
    *,
    ...
    # Read from arm 3's own config file, never from a copy here — the same rule
    # `gate.GateConfig.arm_configs` follows, and for the same reason: a second
    # place to change a hyperparameter is a second place to forget.
    graph_config = GraphArmConfig.from_dict(json.loads(arm_config_path.read_text()))
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="#15: re-run arm 3 with the HAS_DIAGNOSIS relation split collapsed."
    )
    ...
    written = main(args.config, args.arm_config, args.out, use_cache=not args.no_cache)
